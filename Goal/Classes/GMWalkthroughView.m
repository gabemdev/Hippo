...
@implementation GMWalkthroughView
...
- (GMWalkthroughPage *)pageForIndex:(NSInteger)idx {
    return (GMWalkthroughPage *)_pages[idx];
}
...
- (BOOL)showTitleViewForPage:(NSInteger)idx {
    ...
    return ((GMWalkthroughPage *)_pages[idx]).showTitleView;
}
...
- (void)buildScrollView {
    ...
    for (int idx = 0; idx < _pages.count; idx++) {
        GMWalkthroughPage *page = _pages[idx];
...
- (UIView *)viewForPage:(GMWalkthroughPage *)page atXIndex:(CGFloat *)xIndex {
...
- (UIImage *)bgForPage:(NSInteger)idx {
    ...
    return ((GMWalkthroughPage *)_pages[idx]).bgImage;
}
...
- (void)notifyDelegateWithPreviousPage:(NSInteger)previousPageIndex andCurrentPage:(NSInteger)currentPageIndex {
    if(currentPageIndex!=_currentPageIndex && currentPageIndex < _pages.count) {
        GMWalkthroughPage* previousPage = _pages[previousPageIndex];
        GMWalkthroughPage* currentPage = _pages[currentPageIndex];
...
- (void)showInView:(UIView *)view animateDuration:(CGFloat)duration {
    ...
    [UIView animateWithDuration:duration animations:^{
        ...
        GMWalkthroughPage* currentPage = _pages[self.currentPageIndex];
        if(currentPage.onPageDidAppear) currentPage.onPageDidAppear();
