...
typedef NS_ENUM(NSUInteger, PageControlImageType) {
    PageControlImageTypeNormal = 1,
    PageControlImageTypeCurrent,
    PageControlImageTypeMask
};

typedef NS_ENUM(NSUInteger, PageControlStyleDefaults) {
    PageControlStyleDefaultStyleClassic =0,
    PageControlStyleDefaultStyleModern
};

static PageControlStyleDefaults _defaultStyleForSystemVersion;
...
@implementation PageControl
{
    ...
    CGFloat         _measuerdIndicatorHeight;
...
+ (void)initialize
{
    ...
    if ([currSysVer compare:reqSysVer options:NSNumericSearch] != NSOrderedAscending) {
        _defaultStyleForSystemVersion = PageControlStyleDefaultStyleModern;
    } else {
        _defaultStyleForSystemVersion = PageControlStyleDefaultStyleClassic;
    }
}
...
{
    _numberOfPages = 0;
    _tapBehavior = PageControlTapBehaviorStep;
...
- (CGSize)sizeForNumberOfPages:(NSInteger)pageCount
{
    ...
    CGSize size = CGSizeMake(marginSpace + indicatorSpace, _measuerdIndicatorHeight);
    return size;
}
...
- (void)_setImage:(UIImage *)image forPage:(NSInteger)pageIndex type:(PageControlImageType)type
{
    if (pageIndex < 0 || pageIndex >= _numberOfPages) {
        return;
    }
    ...
    switch (type) {
        case PageControlImageTypeCurrent:
            dictionary = self.currentPageImages;
            break;
        case PageControlImageTypeNormal:
            dictionary = self.pageImages;
            break;
        case PageControlImageTypeMask:
...
{
    [self _setImage:image forPage:pageIndex type:PageControlImageTypeNormal];
    [self _updateMeasuredIndicatorSizes];
}

- (void)setCurrentImage:(UIImage *)image forPage:(NSInteger)pageIndex
{
    [self _setImage:image forPage:pageIndex type:PageControlImageTypeCurrent];;
    [self _updateMeasuredIndicatorSizes];
}

- (void)setImageMask:(UIImage *)image forPage:(NSInteger)pageIndex
{
    [self _setImage:image forPage:pageIndex type:PageControlImageTypeMask];
...
- (id)_imageForPage:(NSInteger)pageIndex type:(PageControlImageType)type
{
    if (pageIndex < 0 || pageIndex >= _numberOfPages) {
        return nil;
    }
...
{
    return [self _imageForPage:pageIndex type:PageControlImageTypeNormal];
}

- (UIImage *)currentImageForPage:(NSInteger)pageIndex
{
    return [self _imageForPage:pageIndex type:PageControlImageTypeCurrent];
}

- (UIImage *)imageMaskForPage:(NSInteger)pageIndex
{
    return [self _imageForPage:pageIndex type:PageControlImageTypeMask];
}
...
- (CGSize)intrinsicContentSize
{
    ...
    CGSize intrinsicContentSize = CGSizeMake(UIViewNoIntrinsicMetric, MAX(_measuerdIndicatorHeight, _minHeight));
...
- (void)setStyleWithDefaults:(PageControlStyleDefaults)defaultStyle
{
    switch (defaultStyle) {
        case PageControlStyleDefaultStyleModern:
            ...
            break;
        case PageControlStyleDefaultStyleClassic:
        default:
...
- (void)_updateMeasuredIndicatorSizeWithSize:(CGSize)size
{
    ...
    _measuerdIndicatorHeight = MAX(_measuerdIndicatorHeight, size.height);
}

- (void)_updateMeasuredIndicatorSizes
{
    _measuredIndicatorWidth = _indicatorDiameter;
    _measuerdIndicatorHeight = _indicatorDiameter;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (PageControlTapBehaviorJump == self.tapBehavior) {
